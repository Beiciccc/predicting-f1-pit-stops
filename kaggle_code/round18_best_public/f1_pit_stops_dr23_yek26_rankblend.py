...
def main():
    ...
    yekenot_path, yekenot = find_source(
        lambda text: "yekenot" in text or "realmlp" in text or "pytorch" in text,
        "Yekenot RealMLP PyTorch submission",
        (0.24, 0.26),
        (0.34, 0.36),
    )
    ...
    if sample_ids is not None:
        ...
        if not yekenot[ID_COL].equals(sample_ids):
            yekenot = yekenot.set_index(ID_COL).loc[sample_ids].reset_index()

    blended_rank = 0.995 * rank_norm(dataregressor[TARGET_COL]) + 0.005 * rank_norm(yekenot[TARGET_COL])
    ...
    print(f"yekenot source: {yekenot_path}")
