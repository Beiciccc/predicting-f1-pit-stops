...
def main():
    ...
    sarvesh_path, sarvesh = find_source(
        lambda text: "sarvesh" in text or "eda-to-podium" in text or "podium" in text,
        "Sarvesh EDA-to-podium submission",
        (0.2075, 0.2087),
        (0.3008, 0.3024),
    )
    ...
    blended_rank = 0.99 * rank_norm(dataregressor[TARGET_COL]) + 0.01 * rank_norm(sarvesh[TARGET_COL])
