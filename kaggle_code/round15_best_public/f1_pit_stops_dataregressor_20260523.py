...
def find_dataregressor_submission():
    root = Path("/kaggle/input")
    candidates = [
        root / "notebooks/azzamradman/knock-the-blender-with-one-liner/submission.csv",
        root / "knock-the-blender-with-one-liner/submission.csv",
    ]
    if root.exists():
        for path in sorted(root.rglob("submission.csv")):
            text = str(path).lower()
            if "knock" in text or "blender" in text or "azzam" in text:
                candidates.append(path)
        for path in sorted(root.rglob("*.csv")):
            text = str(path).lower()
            if "submission" in text and ("knock" in text or "blender" in text or "azzam" in text):
                candidates.append(path)
    ...
    for path in candidates:
        ...
        if len(df) == 188165 and 0.2055 < pred.mean() < 0.2066 and 0.3070 < pred.std() < 0.3082:
            return path, df
    raise FileNotFoundError("could not identify the DataRegressor public submission")


def main():
    sample_ids = find_sample_ids()
    source_path, source = find_dataregressor_submission()
