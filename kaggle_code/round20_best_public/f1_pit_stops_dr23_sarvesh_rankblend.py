...
def main():
    ...
    sarvesh_path, sarvesh = find_source(
        lambda text: "sarvesh" in text or "blend-f1" in text or "lightgbm-blending" in text,
        "Sarvesh blending submission",
        (0.4990, 0.5010),
        (0.2860, 0.2895),
    )

    dataregressor = align_to_sample(dataregressor, sample_ids)
    sarvesh = align_to_sample(sarvesh, sample_ids)

    blended_rank = 0.999 * rank_norm(dataregressor[TARGET_COL]) + 0.001 * rank_norm(sarvesh[TARGET_COL])
    ...
    print(f"sarvesh source: {sarvesh_path}")
