#!/usr/bin/env python3
"""Submit the 2026-05-24 queue and wait for public scores."""
...
LIVE_RESULTS = ROOT / "docs" / "round16_live_results.csv"


QUEUE = [
    ("ah01_s54_raw_direct.csv", "r16_loop01_ah01_s54_raw_direct"),
    ("ah02_s54_asym_core_direct.csv", "r16_loop02_ah02_s54_asym_core_direct"),
    ("ah03_s54_ultra_narrow_direct.csv", "r16_loop03_ah03_s54_ultra_narrow_direct"),
    ("ah04_s54_micro_booster_direct.csv", "r16_loop04_ah04_s54_micro_booster_direct"),
    ("ah05_rasulbek_best7_direct.csv", "r16_loop05_ah05_rasulbek_best7_direct"),
    ("ah06_prob_dr23_s54raw_50_50.csv", "r16_loop06_ah06_prob_dr23_s54raw"),
    ("ah07_prob_ag08_s54raw_995_005.csv", "r16_loop07_ah07_prob_ag08_s54raw"),
    ("ah08_prob_dr23_rasulbek_50_50.csv", "r16_loop08_ah08_prob_dr23_rasulbek"),
    ("ah09_rank_dr23_s54raw_50_50.csv", "r16_loop09_ah09_rank_dr23_s54raw"),
    ("ah10_rank_ag09_s54raw_995_005.csv", "r16_loop10_ah10_rank_ag09_s54raw"),
]
    ...
    for attempt in range(1, 6):
        try:
            out = subprocess.check_output(
                [KAGGLE, "competitions", "submissions", "-c", COMPETITION, "-v", "--page-size", "160"],
                text=True,
...
def submit_one(loop_idx: int, rel: str, description: str) -> SubmissionRow:
    ...
    if existing and (existing[0].status.endswith(".COMPLETE") or existing[0].status.endswith(".ERROR")):
        row = existing[0]
        print(f"\n=== round16 loop {loop_idx}/10 already recorded: {description} {row.status} {row.public_score or 'NA'} ===", flush=True)
        record_live(loop_idx, rel, row)
        return row

    print(f"\n=== round16 loop {loop_idx}/10: {description} ===", flush=True)
    returncode = 1
