#!/usr/bin/env python3
"""Submit the 2026-05-30 queue and wait for public scores."""
...
LIVE_RESULTS = ROOT / "docs" / "round22_live_results.csv"


QUEUE = [
    ("an01_varad30_direct.csv", "r22_loop01_an01_varad30_direct"),
    ("an02_ruihao_gpu_full30_direct.csv", "r22_loop02_an02_ruihao_gpu_full30_direct"),
    ("an03_rank_dr23_varad30_95_05.csv", "r22_loop03_an03_rank_dr23_varad30"),
    ("an04_rank_dr23_varad30_90_10.csv", "r22_loop04_an04_rank_dr23_varad30"),
    ("an05_rank_dr23_varad30_80_20.csv", "r22_loop05_an05_rank_dr23_varad30"),
    ("an06_sarvesh_lgbm30_direct.csv", "r22_loop06_an06_sarvesh_lgbm30_direct"),
    ("an07_rank_dr23_sarvesh_lgbm30_80_20.csv", "r22_loop07_an07_rank_dr23_sarvesh_lgbm30"),
    ("an06_sarvesh_triple30_direct.csv", "r22_loop06_an06_sarvesh_triple30_direct"),
    ("an07_prob_dr23_sarvesh_triple30_80_20.csv", "r22_loop07_an07_prob_dr23_sarvesh_triple30"),
    ("an09_rank_dr23_sarvesh_triple30_98_02.csv", "r22_loop09_an09_rank_dr23_sarvesh_triple30"),
]
...
def submit_one(loop_idx: int, rel: str, description: str) -> SubmissionRow:
    ...
    if existing and (existing[0].status.endswith(".COMPLETE") or existing[0].status.endswith(".ERROR")):
        row = existing[0]
        print(f"\n=== round22 loop {loop_idx}/10 already recorded: {description} {row.status} {row.public_score or 'NA'} ===", flush=True)
        record_live(loop_idx, rel, row)
        return row
    if existing:
        print(f"\n=== round22 loop {loop_idx}/10 already pending: {description} ===", flush=True)
    ...
    print(f"\n=== round22 loop {loop_idx}/10: {description} ===", flush=True)
    returncode = 1
