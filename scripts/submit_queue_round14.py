#!/usr/bin/env python3
"""Submit the 2026-05-22 queue and wait for public scores."""
...
LIVE_RESULTS = ROOT / "docs" / "round14_live_results.csv"


QUEUE = [
    ("af01_dataregressor_20260522_direct.csv", "r14_loop01_af01_dataregressor_20260522"),
    ("af02_prob_max52_datareg22_995_005.csv", "r14_loop02_af02_prob_max52_datareg22"),
    ("af03_prob_max52_datareg22_99_01.csv", "r14_loop03_af03_prob_max52_datareg22"),
    ("af04_prob_s53_datareg22_995_005.csv", "r14_loop04_af04_prob_s53_datareg22"),
    ("af05_prob_tail53_datareg22_995_005.csv", "r14_loop05_af05_prob_tail53_datareg22"),
    ("af06_rank_max52_datareg22_995_005.csv", "r14_loop06_af06_rank_max52_datareg22"),
    ("af07_rank_max52_datareg22_99_01.csv", "r14_loop07_af07_rank_max52_datareg22"),
    ("af08_rank_s53_datareg22_995_005.csv", "r14_loop08_af08_rank_s53_datareg22"),
    ("af09_prob_max52_s53_50_50.csv", "r14_loop09_af09_prob_max52_s53"),
    ("af10_prob_max52_tail53_50_50.csv", "r14_loop10_af10_prob_max52_tail53"),
]
...
def submit_one(loop_idx: int, rel: str, description: str) -> SubmissionRow:
    ...
    if existing and (existing[0].status.endswith(".COMPLETE") or existing[0].status.endswith(".ERROR")):
        row = existing[0]
        print(f"\n=== round14 loop {loop_idx}/10 already recorded: {description} {row.status} {row.public_score or 'NA'} ===", flush=True)
        record_live(loop_idx, rel, row)
        return row

    print(f"\n=== round14 loop {loop_idx}/10: {description} ===", flush=True)
    returncode = 1
